 ...
 * @LastEditTime: 2020-07-17 20:11:29
 */
...
const requestInterceptor = config => {
  ...
  if (config.url === '/contract/createOrder') {
...
const responseInterceptor = response => {
  ...
  response.data = JSON.parse(CryptoJS.AES.decrypt(response.data.receive, CryptoJS.enc.Utf8.parse("1234123412ABCDEF"),
    {
        iv: CryptoJS.enc.Utf8.parse("ABCDEF1234123412"),
        mode: CryptoJS.mode.CBC,
        padding: CryptoJS.pad.Pkcs7
    }).toString(CryptoJS.enc.Utf8))
  let code = response && (response.status + '').startsWith('20') ? +response.data.code : response.status
  if (errors[code]) {
    errors[code]()
  } else {
    !(code + '').startsWith('20') && Message.error(response.data && response.data.message)
